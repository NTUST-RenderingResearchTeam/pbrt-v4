...
SampledSpectrum TestPathIntegrator::SampleLd(const SurfaceInteraction &intr, const BSDF *bsdf,
                                         SampledWavelengths &lambda,
                                         Sampler sampler) const {
    // Initialize _LightSampleContext_ for light sampling
    LightSampleContext ctx(intr);
...
std::string TestPathIntegrator::ToString() const {
    return StringPrintf("[ TestPathIntegrator maxDepth: %d lightSampler: %s regularize: %s ]",
                        maxDepth, lightSampler, regularize);
}

std::unique_ptr<TestPathIntegrator> TestPathIntegrator::Create(
    const ParameterDictionary &parameters, Camera camera, Sampler sampler,
    Primitive aggregate, std::vector<Light> lights, const FileLoc *loc) {
    int maxDepth = parameters.GetOneInt("maxdepth", 5);
    std::string lightStrategy = parameters.GetOneString("lightsampler", "bvh");
...
std::unique_ptr<Integrator> Integrator::Create(
    ...
    else if (name == "testpath")
        integrator =
            TestPathIntegrator::Create(parameters, camera, sampler, aggregate, lights, loc);
