...
// GPURestirFilm Method Definitions
void GPURestirFilm::AddSample(Point2i pFilm, SampledSpectrum L,
                                  const SampledWavelengths &lambda,
                                  const VisibleSurface *visibleSurface, Float weight) {
    RGB rgb = sensor->ToSensorRGB(L, lambda);
    Float m = std::max({rgb.r, rgb.g, rgb.b});
...
void GPURestirFilm::AddSplat(Point2f p, SampledSpectrum v,
                                 const SampledWavelengths &lambda) {
    // NOTE: same code as RGBFilm::AddSplat()...
    CHECK(!v.HasNaNs());
    RGB rgb = sensor->ToSensorRGB(v, lambda);
...
void GPURestirFilm::WriteImage(ImageMetadata metadata, Float splatScale) {
    Image image = GetImage(&metadata, splatScale);
    LOG_VERBOSE("Writing image %s with bounds %s", filename, pixelBounds);
    image.Write(filename, metadata);
}

void GPURestirFilm::WriteImage(std::string imgName, ImageMetadata metadata,
                                   Float splatScale) {
    Image image = GetImage(&metadata, splatScale);
    LOG_VERBOSE("Writing image %s with bounds %s", imgName, pixelBounds);
    image.Write(imgName, metadata);
...
Image GPURestirFilm::GetImage(ImageMetadata *metadata, Float splatScale) {
    // Convert image to RGB and compute final pixel values
    LOG_VERBOSE("Converting image to RGB and computing final weighted pixel values");
    PixelFormat format = writeFP16 ? PixelFormat::Half : PixelFormat::Float;
    Image image(format, Point2i(pixelBounds.Diagonal()),
...
std::string GPURestirFilm::ToString() const {
    return StringPrintf("[ GPURestirFilm %s outputFromRender: %s applyInverse: %s "
                        "colorSpace: %s maxComponentValue: %f writeFP16: %s ]",
                        BaseToString(), outputFromRender, applyInverse, *colorSpace,
                        maxComponentValue, writeFP16);
}

GPURestirFilm *GPURestirFilm::Create(const ParameterDictionary &parameters,
                                             Float exposureTime,
    ...
    if (!HasExtension(filmBaseParameters.filename, "exr"))
        ErrorExit(loc, "%s: EXR is the only format supported by the GPURestirFilm.",
                  filmBaseParameters.filename);
    ...
    if (coordinateSystem == "camera") {
        ...
        ErrorExit(
            loc,
            "%s: unknown coordinate system for GPURestirFilm. (Expecting \"camera\" "
            "or \"world\".)",
            coordinateSystem);

    printf("Using test GPURestirFilm.\n");

    return alloc.new_object<GPURestirFilm>(filmBaseParameters, outputFromRender,
...
Film Film::Create(const std::string &name, const ParameterDictionary &parameters,
    ...
    if (name == "rgb")
        ...
        film = RestirGBufferFilm::Create(parameters, exposureTime, cameraTransform, filter,
                                   parameters.ColorSpace(), loc, alloc);
    else if (name == "GPURestirFilm")
        film = GPURestirFilm::Create(parameters, exposureTime, cameraTransform,
                                         filter, parameters.ColorSpace(), loc, alloc);
